import { connectDynamic } from '../db-dynamic'
...
/** pool에서 SP 실행 (커넥션 재사용) */
async function executeSPWithPool(
  pool: sql.ConnectionPool,
  spName: string,
  params?: Record<string, unknown>
): Promise<void> {
...
export async function authenticateUser(
  ...
  try {
    ...
    // 2. MAINID DB 커넥션 1회만 열기
    mainPool = await connectDynamic(mainIdDb)

    const c10Users = await queryWithPool<{
    ...
    tenantPool = await connectDynamic(dbConfig)

    // 4. T_SYS_USER + 암호화 설정 병렬 조회 (같은 테넌트 DB)
    const [users, encSettings] = await Promise.all([
      queryWithPool<{
    ...
    // 5. 로그인 로깅 — MAINID DB + 테넌트 DB 병렬 (비차단)
    const ip = clientIp || '0.0.0.0'

    // 로깅은 응답을 기다리지 않고 병렬로 fire-and-forget
    const loggingPromise = Promise.allSettled([
      // MAINID DB 로깅 3건
      (async () => {
      ...
      // 테넌트 DB SP 2건 병렬
      executeSPWithPool(tenantPool!, 'SP_SYS_ACCES_I1', {
        MAKEDATE: new Date(),
        MAKER: userId,
        IPADDRESS: ip,
      }).catch(() => {}),
      executeSPWithPool(tenantPool!, 'SP_SYS_WRKLOG_I1', {
        PROGRAMID: 'MAIN',
        WORKERID: userId,
        WORKTYPE: 'LOGIN',
        IP: ip,
    ...
    await Promise.all([
      mainPool.close().catch(() => {}),
      tenantPool.close().catch(() => {}),
    ])
    mainPool = null
    tenantPool = null
    ...
    // 에러 시에도 커넥션 정리
    if (mainPool) await mainPool.close().catch(() => {})
    if (tenantPool) await tenantPool.close().catch(() => {})
  }
}
...
export async function logLogout(
  ...
  try {
    const ip = clientIp || '0.0.0.0'
    pool = await connectDynamic(dbConfig)
    await executeSPWithPool(pool, 'SP_SYS_WRKLOG_I1', {
      PROGRAMID: 'MAIN',
      WORKERID: userId,
      WORKTYPE: 'LOGOUT',
      IP: ip,
